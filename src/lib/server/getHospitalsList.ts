...
import { LocationType, PatientType } from "../types";
import {
  fetchHospitals,
  getCordsAddress,
  getPatientTSObject,
...
export async function getHospitals(sitaution: string, location: LocationType) {
  ...
  if (!patientTS) {
    ...
    console.log("Could not generate patient object, try again later!!");
    return
  }

  const fetchedHospitals = fetchHospitals(location);

  const rankedHospitals = rankingHospitals(patientTS, fetchedHospitals);
